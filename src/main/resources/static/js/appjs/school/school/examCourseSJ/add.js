			...
			success : function(data) {
				  if (data > 0) {
					...
					$.ajax({
						cache : true,
						type : "POST",
						url :contextPath+ "/school/examCourse/save",
			...
			success : function(data) {
				  if (data > 0) {
					...
					$.ajax({
						cache : true,
						type : "POST",
						url :contextPath+ "/school/examCourseSJ/save",
						data : $('#signupForm').serialize(),// 你的formid
						async : false,
						error : function(request) {
							parent.layer.alert("Connection error");
		...
		success : function(data) {
			...
			for(var i=0;i<data.length;i++){
				if(data[i].id==parent.kaoshi){
					_html+="<option value="+"'"+data[i].id+"'"+" selected = 'selected'>"+data[i].examYear+"年"+data[i].examMonth+"（衔接）"+"</option>";
				}else{
