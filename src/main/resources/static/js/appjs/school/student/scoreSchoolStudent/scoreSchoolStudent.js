...
var prefix = contextPath+"/student/scoreSchoolStudent"
$(function() {
    college();
	load();
});
						...
						columns : [
								{
									...
									formatter : function(value, row, index) {
										var e = '<a class="btn btn-primary btn-sm" href="#" mce_href="#" title="校考课程" onclick="edit(\''
												+ row.studentid
												+ '\')">成绩录入</a> ';
										var d = '<a class="btn btn-warning btn-sm" href="#" title="查看成绩"  mce_href="#" onclick="remove(\''
												+ row.id
												+ '\')">查看成绩</a> ';
...
function edit(id) {
	layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '90%', '90%' ],
		content : prefix + '/schoolCourseList' // iframe的url
	});
}
function remove(id) {
