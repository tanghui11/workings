...
function reLoad() {
	...
	$('#exampleTable').bootstrapTable('refresh',{url : prefix + "/listQu"});
...
function luru(){
	...
	$.ajax({
		url : contextPath+"/student/schoolCourseScoreInput/save",
		type : "post",
		data : {
			'studentid' : rows[0].id,
			'specialityRecordid' : $("#specialityRecordid").val(),
...
function chakan(id,name) {
	...
	layer.open({
		...
		shadeClose : false, 
		area : [ '80%', '80%' ],
		content : contextPath + '/student/schoolCourseScoreInput/schoolScoreList?studentid='+id+"&&specialityRecordid="+$("#specialityRecordid").val()
	});
}
function remove(id) {
