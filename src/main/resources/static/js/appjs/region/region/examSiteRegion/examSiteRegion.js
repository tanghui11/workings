...
var prefix =contextPath+  "/region/examSiteRegion"
$(function() {
    load();
    loadTree();
    //计算机构树div高度
...
function add() {
    ...
    if($("#regionid").val()==""||$("#regionid").val()==0){
		...
		layer.open({
			type : 2,
			title : "添加 【 地州市考办管理 > 考点管理 】",
			maxmin : true,
			shadeClose : false, 
			area : [ '900px', '75%' ],
			content : prefix + '/add?regionid='+$("#regionid").val()
		});
    }
...
function  importData() {
    layer.open({
        ...
        area : [ '600px', '190px' ],
        content : prefix + '/EXimportData' // iframe的url
    });
}