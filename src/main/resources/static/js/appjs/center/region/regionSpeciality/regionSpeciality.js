...
function add() {
	/* sessionStorage.setItem("regionid",$("#regionid").val()); */
	 //localStorage.regionid=$("#regionid").val();
	if($("#regionid").val()==""||$("#regionid").val()==0){
		layer.msg("考区编号不能为空")
	}else{
		layer.open({
		type : 2,
		title : '授权 [ 计划管理 > 考区专业报考设置 ]',
		shadeClose : false, // 点击遮罩关闭层
		area : [ '750px', '345px' ],
		content :prefix + '/add?regionid='+$("#regionid").val() // iframe的url
	});
	}
...
function allWarranty(){
	if($("#regionid").val()==0||$("#regionid").val()==""){
		...
		$.ajax({
			url :contextPath+"/region/regionSpeciality/bathSave",
			type : "post",
			data : {
				'regionid' : $("#regionid").val()
			},
...
function  importData() {
    layer.open({
        ...
        shadeClose : false, // 点击遮罩关闭层
        area : [ '600px', '190px' ],
        content : prefix + '/importData?regionid='+$("#regionid").val() // iframe的url
    });
}
