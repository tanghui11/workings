...
var prefix = contextPath+ "/region/region"
$(function() {
	validateRule();
	loadTree();
    //load();
...
$('#jstree').on("changed.jstree", function (e, data) {
    if (data.selected == -1) {
	   ...
	   $.ajax({
			cache : true,
			type : "get",
			url : contextPath+"/region/region/edit/"+$('#orgid').val(),
			async : false,
		...
		if($("#orgid").val()==""||$("#orgid").val()==0){
			...
			if($("#code").val()!=daima){
				...
				$.ajax({
					cache : true,
					type : "POST",
					url :contextPath+  "/region/region/update",
					data : $('#signupForm').serialize(),// 你的formid
					async : false,
					error : function(request) {
						parent.layer.alert("Connection error");
	...
	if($("#orgid").val()==""||$("#orgid").val()==0){
	 ...
	 layer.confirm('确认要删除吗？', {
            ...
            $.ajax({
				cache : true,
				type : "POST",
				url :contextPath+  "/region/region/remove",
				data :{
					"id":$('#orgid').val(),
					"type":$("#type").val()
					} ,// 你的formid
...
function importData(){
    layer.open({
        ...
        area : [ '600px', '190px' ],
        content : prefix + '/KimportData' // iframe的url
    });
}