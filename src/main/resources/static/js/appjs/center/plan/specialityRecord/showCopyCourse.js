...
function update() {
layer.confirm('你确定将【'+$("#specialitySrc").val()+'】的开考课程拷贝到【'+$("#subjectName").val()+'】的开考课程中吗？', {
            btn : [ '确定', '取消' ]//按钮
        }, function(index) {
           $.ajax({
				cache: true,
				type: "POST",
				url:contextPath+"/plan/specialityRecord/saveCopy",
	...
	/* $.ajax({
		cache : true,
		type : "get",
		url :contextPath+  "/common/searchIfExist?tableName=pla_speciality_record&method=update&updateKey=id&updateKeyValue="+$("#id").val()+"&filedName1=specialityid&filedValue1="+$("#specialityid").val(),
		async : false,
		error : function(request) {
			parent.layer.alert("Connection error");
		},
...
function majorName(){
	  layer.open({
        ...
        area: ['100%', '90%'],
        content:contextPath+  "/plan/specialityRecord/copy/"+$("#id").val(),
    }); 
