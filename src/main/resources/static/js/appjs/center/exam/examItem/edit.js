...
function update() {
	...
	$.ajax({
		cache : true,
		type : "get",
		url :contextPath+ "/common/searchIfExist?tableName=exa_exam_item&method=update&updateKey=id&updateKeyValue="+$("#id").val()+"&filedName1=exam_month&filedValue1="+$("#examMonth").val()+"&filedName2=exam_taskid&filedValue2="+$("#examTaskid").val(),// 你的formid
		async : false,
		...
		success : function(data) {
			  if (data > 0) {
				...
				$.ajax({
					cache : true,
					type : "POST",
					url : contextPath+"/exam/examItem/update",
