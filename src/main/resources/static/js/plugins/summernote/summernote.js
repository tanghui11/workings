...
(function (factory) {
  ...
  */
  ...
  var Editor = function (context) {
    ...
    /**
     * insertFilesOrCallback
     * @param {File[]} files
     */
    this.insertFilesOrCallback = function (files) {
      var callbacks = options.callbacks;
      // If onFileUpload options setted
    ...
    this.insertLink = function (url,name) {
      var linkInfo = {
        text: name,
        url: url,
        urlType: 'relative',
  ...
  var LinkDialog = function (context) {
    ...
    this.initialize = function () {
      ...
      var body = '<div class="form-group note-form-group">' +
                 ...
                 '<div class="form-group note-form-group note-group-select-from-files">' +
                    '<label class="note-form-label">' + lang.link.selectFromFiles + '</label>' +
                    '<input class="note-image-input form-control note-form-control note-input" '+
                    ' type="file" name="files" accept="application/msword,application/pdf,application/x-zip-compressed" multiple="multiple" />' +
                  '</div>' +
    ...
    this.show = function () {
      var data = context.invoke('editor.getLinkInfo');
      context.invoke('editor.saveRange');
      this.showLinkDialog(data).then(function (data) {
        ui.hideDialog(self.$dialog);
        context.invoke('editor.restoreRange');
        if (data.url != undefined && data.url != '') { // image url
          context.invoke('editor.createLink', data);
        } else { // array of files
          context.invoke('editor.insertFilesOrCallback', data);
        }
      }).fail(function () {
        context.invoke('editor.restoreRange');
      });
    ...
    this.showLinkDialog = function (linkInfo) {
      return $.Deferred(function (deferred) {
        var $linkInput = self.$dialog.find('.note-image-input'),
        ...
        ui.onDialogShown(self.$dialog, function () {
          ...
          $linkInput.replaceWith($linkInput.clone()
            .on('change', function () {
