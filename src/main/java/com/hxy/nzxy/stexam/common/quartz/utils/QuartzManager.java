package com.hxy.nzxy.stexam.common.quartz.utils;

import java.util.*;

import com.hxy.nzxy.stexam.common.domain.ScheduleJob;
import com.hxy.nzxy.stexam.common.domain.ScheduleJob;
...
public class QuartzManager {
     ...
     * @param job
     */
    public void addJob(ScheduleJob job, Map<String, Object> param) {
        try {
            // 创建jobDetail实例，绑定Job实现类
            // 指明job的名称，所在组的名称，以及绑定job类
            ...
            SimpleTrigger simpleTrigger = TriggerBuilder.newTrigger()
                    ...
                    .startAt(new Date())
                    .withSchedule(SimpleScheduleBuilder.simpleSchedule().withIntervalInSeconds(3).withRepeatCount(job.getRepeatCount()))//重复执行的次数，因为加入任务的时候马上执行了，所以不需要重复，否则会多一次。
                    .build();
            // 把作业和触发器注册到任务调度中
