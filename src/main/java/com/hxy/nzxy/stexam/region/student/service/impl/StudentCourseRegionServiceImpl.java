...
@Service
public class StudentCourseRegionServiceImpl implements StudentCourseRegionService {
	@Autowired
	private StudentCourseRegionDao studentCourseRegionDao;
	
	@Override
	public StudentCourseDO get(Long id){
		return studentCourseRegionDao.get(id);
	}
	
	@Override
	public List<StudentCourseDO> list(Map<String, Object> map){
		return studentCourseRegionDao.list(map);
	}
	
	@Override
	public int count(Map<String, Object> map){
		return studentCourseRegionDao.count(map);
	}
	
	@Override
	public int save(StudentCourseDO studentCourseRegion){
		return studentCourseRegionDao.save(studentCourseRegion);
	}
	
	@Override
	public int update(StudentCourseDO studentCourseRegion){
		return studentCourseRegionDao.update(studentCourseRegion);
	}
	
	@Override
	public int remove(Long id){
		return studentCourseRegionDao.remove(id);
	}
	
	@Override
	public int batchRemove(Long[] ids){
		return studentCourseRegionDao.batchRemove(ids);
	}

	@Override
	public List<StudentSpecialityDO> listStudent(Query query) {
		return studentCourseRegionDao.listStudent(query);
	}

	@Override
	public int countStudent(Query query) {
		return studentCourseRegionDao.countStudent(query);
	}

	@Override
	public List<ExamCourseDO> listCourse(Query query) {
		return studentCourseRegionDao.listCourse(query);
	}

	@Override
	public int listCourseCount(Query query) {
		return studentCourseRegionDao.listCourseCount(query);
	}

	@Override
	public List<TaskDO> listTask(Map<String, Object> params) {
		return studentCourseRegionDao.listTask(params);
	}
	...
	private String readExcelValue(Workbook wb,File tempFile, HttpServletRequest request) throws ParseException {
		...
		if (StringUtils.isEmpty(errorMsg)) {
			...
			//取出考生已经报考的学生+原因 已经报考
			List<StudentCourseDO> studentCourseList =studentCourseRegionDao.selectstudentCourse(map);
			//根据开考课程编号取出对应的课程编号
			...
			//取出已经有成绩的学生+原因 已考试
			List<StudentCourseDO> studentCjCourseList =studentCourseRegionDao.selectstudentCjCourse(map1);
			...
			if(list.size()>0){
				studentCourseRegionDao.batchSave(list);
	...
	@Override
	public List<ExamCourseDO> listCourseNew(Query query) {
		return studentCourseRegionDao.listCourseNew(query);
	}

	@Override
	public int listCourseCountNew(Query query) {
		return studentCourseRegionDao.listCourseCountNew(query);
	}

	@Override
	public int selectSameExamTime(String[] examCourseid, String studentid) {
		Map map =new HashMap();
		map.put("examCourseid",examCourseid);
		map.put("studentid",studentid);

		return studentCourseRegionDao.selectSameExamTime(map);
	}
}
